...
namespace STPC.DynamicForms.Web.RT.Controllers
{
  ...
  public class PageMathOperationController : Controller
  {
    ...
    public ActionResult List(Guid id)
    {
      ...
      return PartialView("Index", listPageMathOperation);
    }
    ...
    private List<PageMathOperation> GetDataIndex(Guid id, FormPage thisPage)
    {
      ...
      foreach (var pageMathOperation in listPageMathOperation)
      {
        foreach (var field in ListFieldByPage)
        {
          ...
          {
            ViewData["ResultField" + field.Uid] = field.FormFieldName;
          }
        }
      }
    ...
    public ActionResult Create(Guid idPage)
    {
      ...
      var ListFieldByPage2 = from f in _stpcForms.PageFields
                             where f.FormFieldType_Uid == Guid.Parse("FB94C22E-81F6-4055-BE86-95150A1569E1") ||
                             f.FormFieldType_Uid == Guid.Parse("DC8450A4-63B2-4C56-86A4-2B68D19CD5EF") &&
                             f.PanelUid.ToString().Contains(
                             (
                             from p in _stpcForms.Panels where p.Page.Uid == idPage select p.Uid).ToString()
                             )
                             select f;
    ...
    [HttpPost]
    [ValidateAntiForgeryTokenAttribute]
    public ActionResult getCreate(Guid idPage)
    {
      var thisPage = _stpcForms.FormPages.Expand("Panels").Expand("Form").Where((x => x.Uid == idPage)).FirstOrDefault();
      ViewBag.Panels = thisPage.Panels.ToList().OrderBy(e => e.SortOrder);
      ViewBag.Data_FormPageId = thisPage.Uid;
    ...
    public ActionResult Create(string expretion, Guid resultField, Guid panelUid)
    {
      try
      {
        ...
        _AbcRedisCacheManager.Create(_PageMathOperation, false, false, "listPageMathOperation");
    ...
    [HttpPost]
    [ValidateAntiForgeryTokenAttribute]
    public ActionResult GetEdit(Guid id, Guid idPage)
    {
    ...
    public ActionResult Edit(string expretion, Guid resultField, Guid panelUid, Guid id)
    {
      ...
      // guardar cambios
      _stpcForms.SaveChanges();
      _AbcRedisCacheManager.Create(_PageMathOperation, true, false, "listPageMathOperation");
    ...
    public ActionResult Delete(Guid id)
    {
      ...
      try
      {
        ...
        if (_PageMathOperation != null)
        {
          ...
          // guardar cambios
          _stpcForms.SaveChanges();
          _AbcRedisCacheManager.Create(_PageMathOperation, false, true, "listPageMathOperation");
        }
